// Simple Netlify serverless function for Deal or No Deal game API
// This avoids dependency issues with ESM/CJS compatibility
...
exports.handler = async (event, context) => {
  try {
    ...
    // API routes
    if (path.includes('/api/game/new') && method === 'POST') {
    ...
    if (path.includes('/api/game/select-case') && method === 'POST') {
    ...
    if (path.includes('/api/game/open-case') && method === 'POST') {
      const { gameId, caseNumber } = body;
    ...
    if (path.includes('/api/game/banker-offer') && method === 'POST') {
      const { gameId } = body;
    ...
    if (path.includes('/api/game/accept-offer') && method === 'POST') {
      const { gameId } = body;
    ...
    if (path.includes('/api/game/reject-offer') && method === 'POST') {
      const { gameId } = body;
